...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS SCHEDULE (id serial, bank_name varchar, nasabah_name varchar, gender char(25), \
                                                       jenis_tabungan varchar, saldo varchar, alamat text, tanggal_pembuatan date);')
...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO schedule (bank_name, nasabah_name, gender, jenis_tabungan, saldo, alamat, waktu_pembuatan, tanggal_pembuatan) \
                    ...
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE schedule \
                                          ...
                                          saldo=:5, alamat=:6, waktu_pembuatan=:7, tanggal_pembuatan=:8 \
                                          WHERE id=:9;')
                            session.execute(query, {'1':bank_name_baru, '2':nasabah_name_baru, '3':gender_baru, '4':str(jenis_tabungan_baru), 
                                                    '5':saldo_baru, '6':alamat_baru, '7':waktu_pembuatan_baru, '8':tanggal_pembuatan_baru, '9':id})
                            session.commit()
